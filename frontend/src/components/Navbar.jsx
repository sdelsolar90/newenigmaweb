...
function HamburgerButton({ open, toggle }) {
  ...
  return (
    <button
      ...
      aria-expanded={open}
    >
      <span className={`block w-6 h-0.5 rounded-full transition-all duration-300 ${open ? `rotate-45 translate-y-2 ${barColor}` : barColor}`} />
    ...
    <>
      <nav className={`fixed top-0 left-0 right-0 z-50 transition-all duration-300 ${scrolled ? "bg-nav/95 backdrop-blur-lg border-b border-line shadow-sm py-3" : "bg-transparent py-5"}`}>
        <div className="max-w-7xl mx-auto px-6 flex items-center justify-between">
          ...
          <HamburgerButton open={mobileOpen} toggle={() => setMobileOpen(!mobileOpen)} />
        </div>
      </nav>

      <div
        className={`fixed inset-0 z-40 bg-carbon/60 backdrop-blur-sm transition-opacity duration-300 lg:hidden ${mobileOpen ? "opacity-100" : "opacity-0 pointer-events-none"}`}
        onClick={() => setMobileOpen(false)}
      />
      <div className={`fixed top-0 right-0 z-50 h-full w-80 max-w-[85vw] bg-carbon2 border-l border-line shadow-2xl transition-transform duration-300 ease-out lg:hidden ${mobileOpen ? "translate-x-0" : "translate-x-full"}`}>
